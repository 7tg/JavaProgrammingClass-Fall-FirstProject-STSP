...
public class GreedyH {
    public static void main(String[] args) {
        ...
        System.out.print("Please input file name eg.(gsp_turkiye.txt)or(gsp_dunya.txt) :");
        ...
        findNearest(cities,0,0,0,pt);
        long endTime = System.currentTimeMillis();
        pt.println("Number of cities : "+N);
        pt.println("Total Running Time :"+(endTime-startTime)+"ms");
        
        pt.close();

    }

    public static String findNearest(City[] cities,int index,int dep,double total,PrintWriter pt)
    {
        if(cities[index].flag == false)
            return findNearestBack(cities,index,dep,total,pt);
        ...
        if(min[0] == Double.MAX_VALUE)
        {
            ...
            pt.println("\nTotal Calculation :"+cities[(int)min[1]].name+"----- "+(String.format("%.2f",(total * 0.001)))+"km "+"------>"+cities[index].name+"\n");
            return findNearest(cities,index,dep,total,pt);
        }
        ...
        return findNearest(cities,(int)min[1],dep,total += min[0],pt);
    }
    
    public static String findNearestBack(City[] cities,int index,int dep,double total,PrintWriter pt)
    {
        ...
        if(min[0] == Double.MAX_VALUE)
        {
            ...
            return findNearestBack(cities,(int)min[1],dep,total,pt);
        }
        if ((int)min[1] == dep)
        {
             System.out.println(cities[index].name+"["+cities[index].country.trim()+"]"+"---"+(String.format("%.2f",(min[0] * 0.001)))+"km"+"--->"+cities[(int)min[1]].name+"["+cities[(int)min[1]].country.trim()+"]");
            ...
            System.out.println("\nTotal Calculation Back:"+cities[dep].name+"----- "+(String.format("%.2f",(total * 0.001)))+"km "+"------>"+cities[dep].name);
            pt.println("\nTotal Calculation Back:"+cities[dep].name+"----- "+(String.format("%.2f",(total * 0.001)))+"km "+"------>"+cities[dep].name);
            return (String.format("%.2f",(total * 0.001)));
        }
        System.out.println(cities[index].name+"["+cities[index].country.trim()+"]"+"---"+(String.format("%.2f",(min[0] * 0.001)))+"km"+"--->"+cities[(int)min[1]].name+"["+cities[(int)min[1]].country.trim()+"]");
        pt.println(cities[index].name+"["+cities[index].country.trim()+"]"+"---"+(String.format("%.2f",(min[0] * 0.001)))+"km"+"--->"+cities[(int)min[1]].name+"["+cities[(int)min[1]].country.trim()+"]");
        return findNearestBack(cities,(int)min[1],dep,total += min[0],pt);
    }
